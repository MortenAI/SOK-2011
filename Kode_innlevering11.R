...
df <- subset(df_growth2019, select = c("avg_gdpgrowth", "avg_p", "avg_nry", "avg_gx", "avg_gi", "avg_educ", "avg_n", "avg_nsy"))
# Gi beskrivende navn til variablene (i samme rekkefølge som de ligger i datasettet)
labs <- c("Gjennomsnitlig årlig vekstrate i BNP per capita 2000-2019 (%)", "Gjennomsnittlig årlig befolkningsvekst (%)", "Årlig reduksjonsrate i naturressurser (%)",
          "Årlig vekstrate i eksport (%)", "Årlig vekstrate i investeringer (%)", "Gjennomsnittlig antall år i skole (for befolkning 15+ år)",
          "Gjennomsnittlig vekstrate i arbeidskraften (%)", "Gjennomsnittlig netto-sparing (%)") 
...
p_growth_gi <- df_growth2019 %>%
  ...
  ylab("Gjennomsnittlig C%rlig vekst i BNP per innbygger 2000-2019") + 
...
df_growth2019 %>%
  ...
  ggplot(aes(x=gdppc, y=avg_educ, size=poptot, fill=region)) +
  geom_point(alpha=0.5, shape=21, color="black") +
  scale_size(range = c(.1, 24), name="Population (M)") +
  scale_fill_viridis(discrete=TRUE, guide=FALSE, option="A") +
  theme_ipsum() +
  theme(legend.position="bottom") +
  ylab("Gjennomsnittlig år med utdanning") +
  xlab("Gdp per Capita") +
  theme(legend.position = "none")


##--1 VEKSTRATE I BNP - ÅR URDANNING

p1 <- df_growth2019 %>% 
  ggplot(aes(x=avg_educ, y=avg_gdpgrowth, size=poptot, fill=region))+
  geom_point(alpha=0.4, shape=21)+
  scale_size(range = c(.1, 24), guide="none") +
  theme_ipsum()+
  theme(legend.position="none")+
  ylab('BNP per innbygger')+
  xlab('År med utdanning')+
  theme(legend.title=element_blank())+
  labs(subtitle = "Korrelasjon\nBNP vekstrate - Antall år med utdanning")
  #ggtitle("Korrelasjon\nBNP per innbygger (log) - Antall år med utdanning")+

p1



##---2 VEKSTRATE I BNP - SPARING
...
p2 <- df_growth2019 %>% 
  ggplot(aes(x=avg_nsy, y=avg_gdpgrowth, size=poptot, fill=region))+
  geom_point(alpha=0.4, shape=21)+
  scale_size(range = c(.1, 24), guide="none") +
  theme_ipsum()+
  theme(legend.position="none")+
  ylab('BNP per innbygger')+
  xlab('Sparerate av BNI')+
  theme(legend.title=element_blank())+
...
plot_grid(p1, p2, labels=c("Figur 4", "Figur 5"), ncol = 2, nrow = 1)

facet_grid()

##---3 LOGARITMEN AV BNP - SPARING


df_growth2019 %>% 
  ggplot(aes(x=avg_nsy, y=ln_gdppc, size=poptot, fill=region))+
  geom_point(alpha=0.4, shape=21)+
  scale_size(range = c(.1, 24), guide="none") +
  theme_ipsum()+
  theme(legend.position="bottom")+
  ...
  xlab('Sparerate')+
  theme(legend.title=element_blank())+
  ggtitle("Korrelasjon\nBNP per innbygger (log) - Sparerate av BNI")

## --4 LOGARITME AV BNP - BEFOLKNINGSVEKST

df_growth2019 %>% 
  ggplot(aes(x=avg_p, y=ln_gdppc, size=poptot, fill=region))+
  geom_point(alpha=0.4, shape=21)+
  scale_size(range = c(.1, 24), guide="none") +
  theme_ipsum()+
  theme(legend.position="bottom")+
  ...
  theme(legend.title=element_blank())+
  ggtitle("Korrelasjon\nBNP per innbygger (log) - Befolkningsvekst") 

## --5 LOGARITME AV BNP - HUMANKAPITAL

df_growth2019 %>% 
  ggplot(aes(x=avg_educ, y=ln_gdppc, size=poptot, fill=region))+
  geom_point(alpha=0.4, shape=21)+
  scale_size(range = c(.1, 24), guide="none") +
  theme_ipsum()+
  theme(legend.position="bottom")+
...
model3 <- lm(avg_gdpgrowth  ~ avg_nsy + avg_educ + avg_nry + avg_gi + avg_n + avg_p + avg_gx + ln_gdppc0 , data=  df_growth2019 ) #NB: Du må legge inn "Dine" variabler her
summary(model3)
...
no_outliers1 <- subset(df_growth2019, df_growth2019$avg_gi > (Q1gi - 1.5*IQRgi) & df_growth2019$avg_gi < (Q3gi + 1.5*IQRgi) &  df_growth2019$avg_n > (Q1n - 1.5*IQRn) & df_growth2019$avg_n < (Q3n + 1.5*IQRn))
dim(no_outliers1)

model4 <- lm(avg_gdpgrowth  ~ avg_nsy + avg_educ + avg_nry + avg_gi + avg_n + avg_p + avg_gx + ln_gdppc0, data= no_outliers1) #NB: Du mC% legge inn "Dine" variabler her
summary(model4)
